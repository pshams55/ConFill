# https://github.com/NVIDIA/NeMo/blob/main/nemo/collections/common/callbacks/ema.py#L162
...
class EMA(Callback):
    ...
    def __init__(
        ...
        self.is_ema = False  # if we are currently on EMA or not
...
class EMAOptimizer(torch.optim.Optimizer):
    ...
    def _should_update_at_step(self) -> bool:
        is_update_time = self.current_step % self.every_n_steps == 0
        is_step_done = self.pl_module.took_step
        assert is_step_done is not None
        return is_update_time and is_step_done
